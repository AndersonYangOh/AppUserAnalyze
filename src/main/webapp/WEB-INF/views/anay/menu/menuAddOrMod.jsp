...
<div class="u-body">
	<div class="u-title">
		<c:if test="${flag=='1'}"><span>新增菜单</span></c:if>
		<c:if test="${flag=='0'}"><span>修改菜单</span>
		<input type="hidden" name="oldMenuCode" id="oldMenuCode" value="${sysMenu.menuCode}"/>
		</c:if>
	</div>
	<div class="data d2">
	    <div class="left-tree" style="height:420px">
			<ul id="menuTree" class="ztree lt"></ul>
		</div>
	    <div class="right-table">
			<form>
				<div class="biaodan">
					<label class="name" for="">菜单编码：</label>
					<input type="text" class="b-input-text" maxlength="25" name="menuCode" id="menuCode" onkeyup="this.value=this.value.replace(/[^a-zA-Z0-9_]/g,'')" onafterpaste="this.value=this.value.replace(/[^a-zA-Z0-9_]/g,'')" onchange="changeTip()"<c:if test="${flag=='0'}">disabled="disabled"</c:if> value="${sysMenu.menuCode}">
					<span style="color: red;display: none;float: left" id="codeTip">菜单编码不能为空！</span>
				</div>
				<div class="biaodan">
					<label class="name" for="">菜单名称：</label>
					<input type="text" class="b-input-text" maxlength="50" name="menuName" id="menuName" onchange="changeTip()" value="${sysMenu.menuName}" >
					<span style="color: red;display: none;float: left" id="nameTip">菜单名称不能为空！</span>
				</div>
				<div class="biaodan">
					<label class="name" for="">父&nbsp;节&nbsp;点：</label>
					<select class="b-input-text" name="parentList" id="parentList" style="width:408px">
				...
				<div class="biaodan">
					<label class="name" for="">菜单链接：</label>
					<input type="text" class="b-input-text" maxlength="100" name="menuUrl" id="menuUrl" onchange="changeTip()" value="${sysMenu.menuUri}">
					<span style="color: red;display: none;float: left" id="urlTip">菜单Url不能为空！</span>
				</div>
				<div class="biaodan">
					<label class="name" for="">排序：</label>
					<input type="text" class="b-input-text" name="reorder" maxlength="3" id="reorder" onchange="changeTip()" value="${sysMenu.reorder}" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onafterpaste="this.value=this.value.replace(/[^\d]/g,'')">
					<span style="color: red;display: none;float: left" id="reorderTip">排序号不能为空！</span>
				</div>
				<div class="biaodan">
					<label class="name n2" for="">菜单描述：</label>
					<div class="b-input-text b2">
						<textarea name="remark" id="remark" cols="40" rows="5">${sysMenu.remark}</textarea>
					</div>
				</div>
				...
				<div class="button-box-v">
					<c:if test="${flag=='1'}">
						<a  class="button baocun" onclick="saveMenu('1')">保存</a> <a onclick="javascript:history.go(-1);"  class="button quxiao menu">取消</a>
					</c:if>
					<c:if test="${flag=='0'}">
						<a  class="button baocun" onclick="saveMenu('0')">保存</a> <a onclick="javascript:history.go(-1);"  class="button quxiao menu">取消</a>
					</c:if>
				</div>
			</form>
		</div>
