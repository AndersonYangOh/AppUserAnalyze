...
<script type="text/javascript">
	...
	var menuParams = "";
	var btnParams = "";
	function jiaoyanCode(){
		var reg = /^\w+$/;
		if ($("#roleCode").val()==null||$("#roleCode").val()=="") {
            $.jBox.error("角色编码不能为空", '提示');
			return false;
		}else if(!reg.test($("#roleCode").val())) {
            $.jBox.error("角色编码限定为字母、数字或下划线的组合", '提示');
			return false;
		}
		return true;
	}
...
function getFunOperateByMenuCode(event, treeId, treeNode) {
	...
	$.ajax({
		type: "POST",
		url: "funOperate/getFunOperatesByMenuCode.do?menuCode="+treeNode.id,
		datatype: "json",
		success: function(result) {
			if (result!='' && result.length > 0){
				...
				for (var m = 0; m < result.length; m++) {
					if (checkStrInArray(result[m].operateCode)) {//result[m].isChecked
						$("#operate_btn_span").append("<label style=\"font-size:12px\"><input name=\"operateCodes\" type=\"checkbox\" checked=\"checked\" value=\""+result[m].operateCode+"\"> "+result[m].operateDesc+"</label><br />");
					} else {
						$("#operate_btn_span").append("<label style=\"font-size:12px\"><input name=\"operateCodes\" type=\"checkbox\" value=\""+result[m].operateCode+"\"> "+result[m].operateDesc+"</label><br />");
					}
				}
			}else{
				$("#operate_btn_span").html("");
...
<div class="u-body">
	...
	<div class="data d2">
		<form>
			<div class="biaodan">
			    <label class="name n1" for="">角色编码：</label>
			     <input class="b-input-text" id="roleCode" name="roleCode" maxlength="32" type="text" value="${role.roleCode }">
			</div>
			<div class="biaodan">
			    <label class="name n1" for="">角色名：</label>
			    <input class="b-input-text" id="roleName" name="roleName" maxlength="100" type="text" value="${role.roleName }" onblur="jiaoyanName();">
			</div>
			<div class="biaodan">
				<label class="name n1 n2" for="">备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注：</label>
				<div class="b-input-text b2">
					<textarea name="roleDesc" maxlength="500" id="roleDesc" cols="30" rows="5">${role.roleDesc }</textarea>
				</div>
			</div>
			<div class="biaodan">
				<label class="name n1" style="color:#647497;"for="">操作权限设置：</label>
				<div class="role-setting-wrap">
					...
					<div class="box">
			            ...
			            <div class="bd">
							<label style="font-size:12px"><input id="operateBtnAll" name="operateBtnAll" type="checkbox"> 选择所有</label>
							<hr style="border:1px solid #32416c;">
        ...
        <div class="button-box-v">
			<a class="button baocun" onclick="save();">保存</a>
			<a href="javascript:history.go(-1);" class="button quxiao">取消</a>
        </div>
	</div> 
</div>
